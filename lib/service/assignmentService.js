...
const createAssignment = async (body) => {
  ...
  // Verificar que no exista ya esa combinación inspector+ayudante+edificio
  const existe = await db(DatabaseTable.asignacionesAyudante)
    .where({
      id_inspector: parsed.id_inspector,
      id_ayudante: parsed.id_ayudante,
      id_edificio: parsed.id_edificio,
    })
    .first();
