...
const updateUserStatus = async (userId, isActive) => {
  try {
    // Intentamos actualizar directamente en la BD sin usar getUserById
    // ya que getUserById ignora a los usuarios inactivos.
    const updatedUser = await userRepository.updateUserStatus(userId, isActive);
    
    // Si la BD no devolvió nada, significa que el ID realmente no existe
    if (!updatedUser) {
      throw new utils.CustomError(
        AnomalyCode.userDoesNotExist, 
        "Usuario no encontrado"
